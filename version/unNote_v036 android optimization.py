...
def smart_update(window_info, line, line_array, mouse_state, mouse_flip):
    ...
    if len(line_array) < 1 or len(line) < 2:
        pg.display.flip()
    elif mouse_state == 1 and len(line) > 3:
        width = line[-3][0] - line[-2][0]
        height = line[-3][1] - line[-2][1]

        new_content = pg.Rect(get_cursor_pos()[0], get_cursor_pos()[1], width, height)
        if platform() == "android":
            pg.Rect.inflate_ip(new_content, 50, 50)
        else:
            new_content = main_window
        pg.display.update(new_content)
