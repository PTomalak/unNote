    ...
    while continue_running:
        ...
        for event in pg.event.get():
            ...
            # states: 0-up 1-click_down 2-down 3-click_up 0-up 5-dot
                ...
                mouse_state = 1
                if pg.Rect.collidepoint(left_window.get_bounding_rect(), get_cursor_pos()) and max_page == current_page and len(line_array) != 0:
                    max_page += 1
    ...
    if len(line_array) >= 1:
        for i in range(len(rubber)):
            for k in range(len(line_array)):
                for z in range(len(line_array[k])):
                    if closed_segment_intersect(rubber[i][0], rubber[i][0], line_array[k][z][1], line_array[k][z][1]):
                        print(line_array[k])
                        line_array[k] = [[0, 0], [1, 1]]
