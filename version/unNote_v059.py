...
def main():
    ...
    while continue_running:
        ...
        sans_resizable = pg.font.Font(str(path + "FantasqueSansMono-Regular.ttf"), get_screen_size()[0]//54)
        ...
        if tool == 0:
            ...
            top_window.fill(pg.Color("orange"))
        ...
        for event in pg.event.get():
            ...
            elif event.type == pg.KEYDOWN and event.key == K_s or (get_cursor_pos()[0] > (get_screen_size()[0] - 12)):
                if platform() == "win" or platform() == "linux":
                    if tool == 1:
                        tool = 0
                    elif tool == 0:
                    ...
                    if tool == 1 and (get_cursor_pos()[1] > (get_screen_size()[1]/2)):
                        tool = 0
                    elif tool == 0 and (get_cursor_pos()[1] < (get_screen_size()[1]/2)):
                        tool = 1
        ...
        side_text_1 = pg.font.Font.render(sans_resizable, "Next page: left panel", True, white)
        side_text_2 = pg.font.Font.render(sans_resizable, "Prev page: top panel", True, white)
        side_text_3 = pg.font.Font.render(sans_resizable, "Change tool: 'S'", True, white)
        ...
        screen.blit(side_text_2, window_info[1].topleft)
        screen.blit(side_text_1, [window_info[1].midleft[0], window_info[1].midleft[1] - get_screen_size()[0]//54//2])
        screen.blit(side_text_3, [window_info[1].bottomleft[0], window_info[1].bottomleft[1] - get_screen_size()[0]//54 - 2])
...
def delete_line(line_array, rubber):
    ...
    if len(line_array) >= 1:
        ...
        if len(delete) > 1:
            for it in range(len(delete)-1):
                if delete[-it - 1] != -1:
                    line_array.pop(delete[-it - 1])
