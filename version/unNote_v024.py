...
def main():
    ...
    while continue_running:
        ...
        draw_pixel(mouse_state, mouse_flip, main_window)

        cursor_pos = get_cursor_pos()
        current_stroke(cursor_pos, mouse_state)

        text = pg.font.Font.render(font, str(cursor_pos) + " " + str(mouse_state) + " " + str(mouse_flip) + " "
                                   + str(get_cursor_pos()[0] - window_info[0][0]) + " " + str(get_cursor_pos()[1] - window_info[0][1]), True, morpheon)
...
def draw_pixel(mouse_state, mouse_flip, surface):
    window_info = divide_screen()
...
def current_stroke(cursor_position, mouse_state):
    if mouse_state == 0:
        1
