    ...
    while continue_running:
        ...
        for event in pg.event.get():
            ...
            # states: 0-up 1-click_down 2-down 3-click_up 0-up 5-dot
                ...
                mouse_state = 1
                if pg.Rect.collidepoint(left_window.get_bounding_rect(), get_cursor_pos()):
                    new_page(line, line_array, page, current_page)
                    current_page += 1
                elif pg.Rect.collidepoint(top_window.get_bounding_rect(), get_cursor_pos()):
                    previous_page(line, line_array, page, current_page)
                    current_page -= 1
        ...
        text = pg.font.Font.render(font, str(current_page), True, morpheon)
...
def new_page(line, line_array, page, current_page):
    line.clear()
    page.append(line_array)
    line_array.clear()


def previous_page(line, line_array, page, current_page):
    line.clear()
    page.append(line_array)
    line_array.clear()
    for i in page[current_page-1]:
        line_array.append(page[i])
